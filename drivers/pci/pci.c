 ...
 *	@exclusive: whether the region access访问 is exclusive独占的 or not描述的是对资源的访问方式
 *非独占访问适用于那些不会因为并发访问而产生冲突或数据完整性问题的场景。
...
{
	return pci_request_selected_regions(pdev, ((1 << 6) - 1), res_name);//左移6位，就是第6位为1，减1，就是0-5位为1.
}
