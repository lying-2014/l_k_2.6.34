...
static int e100_exec_cmd(struct nic *nic, u8 cmd, dma_addr_t dma_addr)
{
	...
	for (i = 0; i < E100_WAIT_SCB_TIMEOUT; i++) {
		if (likely(!ioread8(&nic->csr->scb.cmd_lo)))
			break;
		cpu_relax();//需要在一个循环中等待某个条件,提供一个提示给处理器，表明当前正处于忙等状态。让cpu更轻松些。
		if (unlikely(i > E100_WAIT_SCB_FAST))//i大的时候，才udelay(5)；i大说明已经疲惫了
			udelay(5);
	}
	if (unlikely(i == E100_WAIT_SCB_TIMEOUT)) {//愣是i走到了最大值
		err = -EAGAIN;
		goto err_unlock;
	}
...
static inline int e100_phy_supports_mii(struct nic *nic)
{
	...
	*/
	return (nic->mdio_ctrl != mdio_ctrl_phy_mii_emulated);//确实是！=的话，就返回true
}
...
static void e100_get_defaults(struct nic *nic)
{
	...
	nic->params.rfds = rfds;//成员的成员；结构体包含结构体。
	nic->params.cbs = cbs;
...
static int e100_poll(struct napi_struct *napi, int budget)
{
	...
	unsigned int work_done = 0;//接收初始值

	e100_rx_clean(nic, &work_done, budget);//接收动作
	e100_tx_clean(nic);//这个是真的清除。为啥在这？发送数据包也会中断。
	...
	const struct pci_device_id *ent)
{
	...
	if (!(netdev = alloc_etherdev(sizeof(struct nic)))) {//返回的是struct net_device的地址；nic的地址通过函数获取
	...
	netdev->watchdog_timeo = E100_WATCHDOG_PERIOD;//使用的是2HZ，而不是具体的时间
	strncpy(netdev->name, pci_name(pdev), sizeof(netdev->name) - 1);

	nic = netdev_priv(netdev);
	netif_napi_add(netdev, &nic->napi, e100_poll, E100_NAPI_WEIGHT);//对nic->napi进行实例初始化；一次最多这么多，E100_NAPI_WEIGHT。
	nic->netdev = netdev;
	nic->pdev = pdev;
	nic->msg_enable = (1 << debug) - 1;
	nic->mdio_ctrl = mdio_ctrl_hw;//for mdio_write and mdio_read
	...
	if (ent->driver_data)
		nic->flags |= ich;//枚举类型的成员值可以单独使用，也可以不在枚举类型的范围内进行赋值（不推荐）。
	else
		nic->flags &= ~ich;
...
{
	if (((1 << debug) - 1) & NETIF_MSG_DRV) {//很巧妙，左移几位，就有几个1.
